...
# Функция для поиска текстового описания по запросу через Яндекс
def get_description_from_yandex(query):
    ...
    try:
        ...
        description = None
        desc_tag = soup.find("div", {"class": "text-container typo typo_text_m typo_line_m organic__text"})
...
for index, item_name in enumerate(items, start=2):
    if item_name:
        ...
        description = get_description_from_yandex(item_name)
        if description:
